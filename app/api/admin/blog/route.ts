...
import {
  getAllBlogPosts,
  readBlogPostsFromBlobRaw,
  saveBlogPostsToBlob,
} from "@/lib/blogPostsPersistence";
...
const verifyBlogBlobWrite = process.env.BLOB_READ_WRITE_TOKEN
  ? async (written: BlogPost[]) => {
      const blob = await readBlogPostsFromBlobRaw();
      return (
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await writeBlobJsonArrayWithRetry({
      read: getAllBlogPosts,
      write: persistPosts,
      verifyAfterWrite: verifyBlogBlobWrite,
      mutate: (existing) => {
        const slugRaw =
        ...
        newPost = {
          ...
          publishedAt: published
            ? (body.publishedAt as string) || now
            : body.publishedAt
  ...
  try {
    ...
    await writeBlobJsonArrayWithRetry({
      ...
      mutate: (existing) => {
        ...
        if (index === -1) {
          throw new MutationHttpError(NextResponse.json({ error: "Post not found" }, { status: 404 }));
        }
