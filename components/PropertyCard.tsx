...
export default function PropertyCard({ property, detailSlug, viewReturnPath }: PropertyCardProps) {
  ...
  const buildTeaser = (): string | null => {
    ...
    const scoreSentence = (s: string) => {
      ...
      if (
        /walking distance|minutes? to|close to|nearby|google maps|location|area\b|center\b|ubud\b|penestanan\b|peliatan\b|lodtunduh\b|petulu\b|pejeng\b|sayan\b|singakerta\b|tegallalang\b|gentong\b|mas\b/.test(
          t
        )
      ) {
        score -= 2;
